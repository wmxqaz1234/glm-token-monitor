...
## 验收标准
- [ ] 卡皮巴拉宠物能正确显示在主窗口
- [ ] 5种佛系动画（idle、orange、spa、munch、stare）正常工作
- [ ] 动作每25秒自动切换
- [ ] 宠物状态变化时颜色正确更新
- [ ] 可以通过配置切换到卡皮巴拉宠物
...
**要求**:
- 在 script 部分导入 Capybara 组件
- 在模板中添加 `v-else-if="petType === 'capybara'"` 分支
- 传递 color、strokeColor、state、width、height props

### Task 3: 验证类型定义已完整
**验收**: `src/types/config.ts` 中 PetType 包含 'capybara'，CapybaraAction 类型已定义

**要求**:
- PetType = 'spirit' | 'ghost' | 'capybara'
- CapybaraAction 包含所有5种动作

### Task 4: 验证 usePetAction.ts 已更新
**验收**: `src/composables/usePetAction.ts` 支持 capybara 类型

**要求**:
- CAPYBARA_ACTIONS 数组包含所有5种动作
- availableActions computed 支持 capybara
- setPetType 函数支持 capybara

### Task 5: 运行开发服务器验证
**验收**: npm run tauri:dev 成功启动，卡皮巴拉正常显示
